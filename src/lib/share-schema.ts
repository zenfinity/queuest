...
function parseShareItem(raw: unknown): ShareItem | null {
	...
	const providers = Array.isArray(item.providers)
		? item.providers.slice(0, 50).map((p: unknown) => {
	...
	const seasons = Array.isArray(item.seasons)
		? item.seasons.slice(0, 100).map((s: unknown) => {
...
export function parseSharePayload(raw: unknown): SharePayload {
	...
	const queue_name = typeof payload.queue_name === 'string' ? payload.queue_name.slice(0, 100) : undefined;
...
export function parseImportBackup(raw: unknown): {
	...
	if (Array.isArray(payload) && payload.length > 0 && payload[0] && typeof payload[0] === 'object') {
		for (const item of payload.slice(0, 5000)) {
			if (item && typeof item === 'object') {
				...
				if (typeof obj.tmdb_id === 'number' && (obj.media_type === 'movie' || obj.media_type === 'tv') && typeof obj.title === 'string') {
	...
	const prefs = payload.prefs && typeof payload.prefs === 'object' ? (payload.prefs as Record<string, unknown>) : {};
	const parsed_prefs = {
		...(typeof prefs.theme === 'string' && ['light', 'dark'].includes(prefs.theme) ? { theme: prefs.theme } : {}),
		...(typeof prefs.weeklyHours === 'number' && prefs.weeklyHours > 0 ? { weeklyHours: Math.round(prefs.weeklyHours) } : {}),
		...(typeof prefs.weeksPerMonth === 'number' && prefs.weeksPerMonth > 0 ? { weeksPerMonth: Math.round(prefs.weeksPerMonth) } : {}),
		...(typeof prefs.budget === 'number' && prefs.budget > 0 ? { budget: Math.round(prefs.budget) } : {}),
		...
		...(prefs.queueColors && typeof prefs.queueColors === 'object'
			...
			: {}),
		...(typeof prefs.sort === 'string' && ['added', 'runtime', 'title'].includes(prefs.sort) ? { sort: prefs.sort } : {}),
		...(typeof prefs.view === 'string' && ['grid', 'list', 'gantt'].includes(prefs.view) ? { view: prefs.view } : {})
	};

	const services = Array.isArray(payload.services)
		? payload.services.slice(0, 100).map((s: unknown) => {
				if (!s || typeof s !== 'object') return null;
